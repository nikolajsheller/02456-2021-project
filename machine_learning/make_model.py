...
from machine_learning.early_stopping import EarlyStopping
import wandb
...
def train_model(lstm, criterion, optimizer, config, verbose=0):
    ...
    X_train, X_test, X_valid, y_train, y_test, y_valid = data_loader(no_files=config.no_files)
    ...
    for epoch in range(config.epochs):
        ...
        for _ in range(len(X_train)):
            ...
            while data.shape[0] > i_batch + config.no_batches:
                ...
                loss.backward()  # calculates the loss of the loss function#retain_graph=True
...
class LSTM(nn.Module):
    def __init__(self, num_classes, input_size, lstm_hidden_size, linear_hidden_size, num_layers, lstm_dropbout,
        ...
        self.linear_1 = nn.Linear(lstm_hidden_size, linear_hidden_size)  # fully connected dense layer
